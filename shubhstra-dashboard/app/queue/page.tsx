    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="ml-64 p-8">
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold text-gray-800">Queue Management</h2>
            <p className="text-gray-600">Live waiting room display</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-gradient-to-br from-primary-600 to-primary-700 rounded-2xl shadow-2xl p-12 mb-8 text-center">
              <div className="text-white">
                <p className="text-2xl font-medium mb-4 opacity-90">Current Token</p>
                {currentToken ? (
                  <div className="text-9xl font-bold mb-4">#{currentToken}</div>
                ) : (
                  <div className="text-6xl font-bold mb-4 opacity-75">No Active Token</div>
                )}
            ...
            <div className="bg-white rounded-lg shadow-md p-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-6">Next in Queue</h3>
              
              {waitingPatients.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  <Users className="w-16 h-16 mx-auto mb-4 opacity-50" />
                  <p className="text-xl">No patients in queue</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {waitingPatients.slice(0, 5).map((item, index) => (
                    <div
                      key={item.id}
                      className="flex items-center justify-between p-6 bg-gray-50 rounded-lg border-l-4 border-primary-500 hover:bg-gray-100 transition-colors"
                    >
                      <div className="flex items-center space-x-6">
                        <div className="bg-primary-100 text-primary-700 rounded-full w-16 h-16 flex items-center justify-center">
                          <span className="text-2xl font-bold">#{item.token_number}</span>
                        </div>
                        <div>
                          <p className="text-xl font-semibold text-gray-800">
                            {item.patients?.name || 'Unknown Patient'}
                          </p>
                          <p className="text-gray-500">{item.patients?.phone_number}</p>
                        </div>
                      </div>
                      {index === 0 && (
